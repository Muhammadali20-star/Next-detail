      ...
      {data?.users?.map((user: IUsers) => (
        <div key={user.id} className="bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition duration-300">
          <div className="p-5 flex flex-col items-center space-y-3">
            <Image  src={user.image} alt={user.firstName} width={120} height={120} className="rounded-full border-4 border-purple-500 shadow-md"/>
            ...
            <button onClick={()=> router.push(`/user/${user.id}`)} className="mt-2 bg-purple-600 hover:bg-purple-700 text-white text-sm px-4 py-1.5 rounded-full shadow hover:shadow-lg transition">View Profile</button>
          </div>
        </div>
      ))}
      </div>
